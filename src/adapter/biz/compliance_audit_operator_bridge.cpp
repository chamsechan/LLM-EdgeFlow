...
void RegisterComplianceAuditBridge() {
  ...
  desc.convert_sample_output =
    ...
    int ret =
        CopyToOperatorString(in_dto->risk_level.c_str(), out->risk_level,
                             spec.GetCapacity("risk_level"), "risk_level", err);
    if (ret != 0) return ret;

    ret = CopyToOperatorString(in_dto->matched_policy_clause.c_str(),
                               out->matched_policy_clause,
                               spec.GetCapacity("matched_policy_clause"),
