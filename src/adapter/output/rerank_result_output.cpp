...
int EncodeCAbiRerankResult(AlgContext* context,
  ...
  for (int i = 0; i < count; ++i) {
    auto* out_ptr = destination->GetCAbi<CompanyRerankBatchOutputStruct>(i);
...
OutputConverterDefinition MakeCAbiRerankResultOutputConverter() {
  OutputConverterDefinition def;
  def.converter_id = "rerank_result.plain.cabi.v1";
  def.transport = "cabi";
  def.schema_id = "rerank_result.plain.response";
  def.schema_version = 1;
  def.external_type = "CompanyRerankBatchOutputStruct";
  def.cardinality = "1:1";
  ...
  def.encode_fn = &EncodeCAbiRerankResult;
  return def;
}
...
REGISTER_OUTPUT_CONVERTER(MakeCAbiRerankResultOutputConverter());
