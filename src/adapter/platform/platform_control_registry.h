...
/**
 * @brief 平台控制指令解析与参数适配器 (将平台枚举与参数分发到
 * Pipeline::Control)
 */
class PlatformControlRegistry {
 public:
  /**
   * @brief 解析平台控制参数为标准 JSON 控制字符串
   * @param[in] command 控制命令枚举
   * @param[in] control_param 平台控制参数指针
   ...
   */
  static int ResolveControlParam(llm_edgeflow::platform::ControlCommand command,
                                 void* control_param, int* out_cmd_id,
