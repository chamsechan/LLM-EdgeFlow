...
int DecodeCAbiRerankInput(const ExternalInputBatchView& source,
  ...
  for (size_t i = 0; i < source.count; ++i) {
    const auto* in_rerank = source.GetCAbi<CompanyRerankBatchInputStruct>(i);
    if (!AdapterValidationHelper::RequireNotNull(
            "inputs[i]", in_rerank, static_cast<int>(i),
            options.converter_id.c_str(), status)) {
      return COMPANY_ALG_ERR_INVALID_INPUT;
    }

    if (!AdapterValidationHelper::RequireBoundedString(
            "inputs[i].query_text", in_rerank->query_text, kMaxTextLen,
            static_cast<int>(i), options.converter_id.c_str(), status)) {
      return COMPANY_ALG_ERR_INVALID_INPUT;
    }

    if (!AdapterValidationHelper::RequireRange(
            "inputs[i].candidate_count", in_rerank->candidate_count, 1, 8,
            static_cast<int>(i), options.converter_id.c_str(), status)) {
      return COMPANY_ALG_ERR_INVALID_INPUT;
    }
...
InputConverterDefinition MakeCAbiRerankInputConverter() {
  InputConverterDefinition def;
  def.converter_id = "rerank.plain.cabi.v1";
  def.transport = "cabi";
  def.schema_id = "rerank.plain.request";
  def.schema_version = 1;
  def.external_type = "CompanyRerankBatchInputStruct";
  ...
  def.decode_fn = &DecodeCAbiRerankInput;
  return def;
}
...
REGISTER_INPUT_CONVERTER(MakeCAbiRerankInputConverter());
