...
int DecodeCAbiImageQueryInput(const ExternalInputBatchView& source,
  ...
  for (size_t i = 0; i < source.count; ++i) {
    const auto* in_ocr = source.GetCAbi<CompanyOcrDocInputStruct>(i);
    if (!AdapterValidationHelper::RequireNotNull(
            "inputs[i]", in_ocr, static_cast<int>(i),
            options.converter_id.c_str(), status)) {
      return COMPANY_ALG_ERR_INVALID_INPUT;
    }

    if (!AdapterValidationHelper::RequireBoundedString(
            "inputs[i].image_path", in_ocr->image_path, kMaxPathLen,
            static_cast<int>(i), options.converter_id.c_str(), status)) {
      return COMPANY_ALG_ERR_INVALID_INPUT;
    }
    if (!AdapterValidationHelper::RequireBoundedString(
            "inputs[i].query_prompt", in_ocr->query_prompt, kMaxQueryLen,
            static_cast<int>(i), options.converter_id.c_str(), status)) {
      return COMPANY_ALG_ERR_INVALID_INPUT;
    }
...
InputConverterDefinition MakeCAbiImageQueryInputConverter() {
  InputConverterDefinition def;
  def.converter_id = "image_query.plain.cabi.v1";
  def.transport = "cabi";
  def.schema_id = "image_query.plain.request";
  def.schema_version = 1;
  def.external_type = "CompanyOcrDocInputStruct";
  ...
  def.decode_fn = &DecodeCAbiImageQueryInput;
  return def;
}
...
REGISTER_INPUT_CONVERTER(MakeCAbiImageQueryInputConverter());
