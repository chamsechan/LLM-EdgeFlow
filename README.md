...
- **v1.3.0** *(2026-08)*
  - 📐 **原生 DAG 拓扑排序调度器**：核心 `Pipeline` 全面支持显式依赖声明 (`depends_on`)、Kahn 算法自动化拓扑重排与环路死锁检测 (`CycleDetection`)，100% 向后兼容存量线性配置。
  - ⚡ **8 大 CTest 自动化测试套件**：新增 `DagPipelineTest`，全量覆盖乱序重排、钻石汇聚图、环路死锁与自环拦截测试。
- **v1.2.0** *(2026-08)*
