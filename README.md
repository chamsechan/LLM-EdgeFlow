...
## 📐 框架类图与 UML 架构设计 (Class Diagram & Architecture)
...
<details>
<summary><b>📊 查看 GitHub 原生渲染类图 (Mermaid Class Diagram)</b></summary>
...
classDiagram
    ...
    class Company_C_ABI {
        <<C API Export>>
        +Alg_Init() int
        +Alg_Create(void** hndl, ParamCreate*) int
        +Alg_Process(void* hndl, in, out) int
        +Alg_Control(void* hndl, ParamControl*) int
        +Alg_Destroy(void* hndl) int
        +Alg_DeInit() int
    }

    class AlgHandleInstance {
    ...
    %% Layer 2
    class Pipeline {
        -vector~unique_ptr~INode~~ nodes_
        -SessionContext session_context_
        -string business_name_
        +BuildFromConfigFile(string) bool
        +BuildFromJson(json) bool
        +Execute(AlgContext*) int
        +Control(int, string) int
    }

    class AlgContext {
        <<Request Blackboard>>
        -unordered_map~string, any~ blackboard_
        -int error_code_
        -string error_message_
        +Set(string, T) void
        +Get(string) T*
        +Has(string) bool
        +SetError(int, string) void
        +Clear() void
    }
    ...
    class ModelManager {
        -unordered_map~string, shared_ptr~IModelEngine~~ models_
        +RegisterModel(string, shared_ptr~IModelEngine~) void
        +GetModel(string) shared_ptr~T~
    ...
    class INode {
        <<interface>>
        +Init(json, SessionContext*) bool*
        +Process(AlgContext*) int*
        +Control(int, json) int*
        +Name() string*
    }

    class PreProcessNodes {
        +DocChunkPreNode
        +ImagePreNode
        +AudioFeaturePreNode
        +KeywordMatcherNode
    ...
    class IModelEngine {
        <<interface>>
        +Load(string, json) bool*
        +GetMaxBatchSize() size_t*
        +EngineType() string*
    }

    class IEmbeddingEngine {
        <<interface>>
        +InferTraceableBatch(inputs, outputs) int*
    }

    class ILlmEngine {
        <<interface>>
        +Generate(prompt, opt, out) int*
        +InferTraceableBatch(prompts, opt, out) int*
    }

    class IRerankEngine {
        <<interface>>
        +ScoreTraceableBatch(pairs, scores) int*
    }

    class IOcrEngine {
        <<interface>>
        +InferTraceableBatch(images, boxes) int*
    }

    class IAudioAsrEngine {
        <<interface>>
        +InferTraceableBatch(audios, transcripts) int*
    }

    class FixedBatchExecutor {
        <<Generic Template>>
        +Execute(inputs, max_batch, pad, kernel, outputs)$ int
    }

    %% Relationships
    Company_C_ABI ..> AlgHandleInstance : Manages
    AlgHandleInstance *-- Pipeline
    Pipeline *-- SessionContext
    Pipeline *-- INode : Orchestrates
    SessionContext *-- ModelManager
    ModelManager o-- IModelEngine

    INode <|.. PreProcessNodes
    INode <|.. InferNodes
    INode <|.. PostProcessNodes

    InferNodes ..> AlgContext : Read / Write Features
    InferNodes ..> ModelManager : Request Engine
    ...
    IModelEngine ..> FixedBatchExecutor : Uses
    FixedBatchExecutor ..> TraceableItem : Slices / Pads / Aligns
```

</details>

<details>
<summary><b>📄 查看 PlantUML 原始代码 (doc/architecture.puml)</b></summary>
...
package "Layer 1: C ABI 适配层" {
  class "Company C ABI Exports" as C_ABI
  class "AlgHandleInstance" as HandleInst
}
package "Layer 2: 框架调度与黑板" {
  class Pipeline
  class AlgContext
  class SessionContext
  class ModelManager
  class "TraceableItem<T>" as TraceableItem
}
package "Layer 3: 业务专属算子池" {
  interface INode
  class PreProcessNodes
  class InferNodes
  class PostProcessNodes
}
package "Layer 4: 异构引擎与硬件批调度" {
...
INode <|.. InferNodes
INode <|.. PostProcessNodes
InferNodes ..> AlgContext
InferNodes ..> ModelManager
IModelEngine <|-- IEmbeddingEngine
IModelEngine <|-- ILlmEngine
IModelEngine <|-- IRerankEngine
IModelEngine <|-- IOcrEngine
IModelEngine <|-- IAudioAsrEngine
IModelEngine ..> FixedBatchExecutor
FixedBatchExecutor ..> TraceableItem
@enduml
```
