...
- 删除必需输入的同名隐式连接和模型实例名默认绑定；缺少必需输入或模型引用明确报错，可选输入缺省保持未连接。
- 删除模型配置中的 `capability`，改从注册定义获取；并发配置只保留 `max_parallel_workers`，默认 1。
- 配置、Schema、CLI 和 Studio 一次迁移，移除补数据依赖及恢复隐式连接操作；旧 `ports`、`execution_mode`、模型 `capability` 字段拒绝解析。
- 保留数据 key、额外依赖、输出映射和现有运行时资源安全检查，不新增引用语法或兼容格式。

## 2026-09-23 新人入口与接入名称信息（RFC-0070）

- 新人文档收敛为已有能力编排、自定义 Node 和外部业务契约三条路径，优先复用现有 Recipe、脚手架和无模型样例。
- Catalog v4 补充外部槽的宿主类型、类型后缀和有效外部键后缀；Studio 运行页展示契约、Demo、Binding、Converter 与槽位的对应关系。
- 删除工具中的固定业务到 Binding 映射，未带部署的方案复用所选 Profile 的显式绑定，已有绑定继续交原生校验。
- 修复 Studio 模型改名后当前 Pipeline 的部署路径仍引用旧 ID 的问题，保留路径值和其他模型覆盖。
...
- 抽取重复的批次、槽访问、文本安全复制、结果读取和容量写入；文档问答、多槽 OCR、PCM、审核及候选排名保留各自语义。
- 字符串转换使用实际池规格，删除业务内重复容量默认值；手工测试视图显式描述真实缓冲容量。
- `alg_pipeline_tool` 增加 `describe-model` / `describe-backend`，复用 Catalog；接入指南覆盖八类业务及现有容量预检流程。
- 保持公共 Operator、请求响应、配置、Catalog 及池生命周期行为；源码扩展辅助接口随框架重新编译。

## 2026-09-21 业务开发易用性修复（RFC-0065）

- 修复可选输入槽的帧索引错位；输出缺少内部数据与外部容量不足使用不同错误码。
- Adapter 复用类型化 key/port，开放内置输入绑定和常见池化输出辅助函数；多槽测试夹具支持池规格。
- Node 脚手架默认自动选择已有函数式 Map、LLM、Embedding、Control 模板。
- Model 能力接口追加可选诊断参数，Node 保留 Backend 原因；共享资源创建失败传给等待者，后续调用可重试。
- 保持现有目录职责，通用分配实现留在 Adapter/Operator；源码扩展随新接口重新编译。
...
- 关闭 BLAS 时不再导入缓存目录残留的 BLAS 归档，并检查基础 GGML 归档是否完整。
...
- 正确展开服务端返回的对象形式 Validator 诊断，保留保存/运行失败的节点定位。

## 2026-09-10 Pipeline Studio 操作流程改进

- 新建提供直接入口；浏览时可展开只读属性，画布工具栏支持直接校验和进入运行页。
- 保存、校验、运行及节点切换遇到表单草稿时可“应用并继续”，非法输入保留并定位；继续保留显式应用、撤销与放弃修改。
- 运行结果绑定提交时的方案和运行设置，修改后提示旧版本，跨文档异步返回保持隔离；样本统计和逐请求输出优先展示，日志与 JSON 按需展开。
- 服务端返回实际保存范围，配套方案重新打开后仍正确显示 JSON + `.conf`；保存冲突和校验错误持续展示，运行与保存的部署解析结果分别呈现。
- 在现有 Studio 测试入口补充运行快照、保存范围及可选真实浏览器回归，覆盖桌面布局、草稿保护、保存冲突和实际 Demo 输出。

## 2026-09-10 Pipeline Studio Web 启动修复

- 修复方案列表接口调用不存在的方法及前端读取错误字段，恢复启动和保存后的方案列表刷新。
- API 请求保留页面所在目录，支持带路径前缀的端口代理；非 JSON 响应显示接口路径、HTTP 状态和响应摘要，避免只报 `Unexpected token`。

## 2026-09-10 构建扩展目录迁移

- 仓库自有 `cmake/` 目录重命名为 `cmake_ext/`，为公司内部构建目录保留名称；同步构建入口、模块依赖、检查脚本、测试夹具和现行开发文档。
