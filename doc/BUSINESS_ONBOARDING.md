...
2. **新的外部输入/输出结构**：在接入适配层增加业务 key/契约和注册 Adapter。`Unpack` 负责外部结构转 Traceable Batch；`PackTyped<Output>` 只写一次业务结果校验和映射。参考 `src/adapter/adapters/doc_qa_adapter.cpp`。
3. **需要同时提供 C ABI 和 Operator**：Adapter 继承 `ResultPackingAdapter<Adapter, COutput, Result>`。COutput 使用公共固定数组契约，Result 使用接入适配层自有字符串。字段字符串通过 `CopyResultString` 写出：固定 C 数组不足返回错误，可变长结果完整保存。
