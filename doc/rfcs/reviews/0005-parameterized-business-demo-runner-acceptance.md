...
- **复验结论（修复分支 `fix/rfc-0005-acceptance-p1-p2`）**：**验收通过 (PASS)**。全部 3 项 P1 阻塞项及 4 项 P2 质量改进项已全部完成修复并通过严格回归测试，待用户最终确认后合入主干。

当前架构已完全达成 RFC-0005 的目标：7 个业务 Demo 已拆分到独立文件，共用参数解析、Profile、注册表、数据读取、Operator Runner 和结果写入能力；核心 Platform Operator、C ABI、Pipeline、Node 与 Engine 契约未被修改。常规构建、21 组 CTest、跨目录 Demo 调度及 6 阶段全量回归均 100% 通过。
...
- Demo 只调用公开 Platform Operator API，没有绕过公开接口直接访问 Pipeline、Node 或 Engine。
- `CompanyAlgBizType`、`ChipType`、`ControlCommand`、`OperatorFunc` 和 `NamedIoBatch` 等核心契约保持不变。
- CMake 和测试脚本已纳入新的 Demo 源文件与测试。

### 3.2 部分实现或与设计语义存在偏差

- CLI 覆盖 Profile 的优先级只在“CLI 值不同于默认值”时生效，无法区分“用户显式传入默认值”和“用户未传入”。
- `batch_size` 当前被用于扩大 Operator 的最大 Batch，而不是限制、分块或拒绝超量数据集，未形成稳定的批处理语义。
- Profile 中的 `suite` 字段没有成为 Smoke、Real、All 套件的唯一事实源；C++ 和 Shell 中仍维护硬编码列表。
- 显式指定的数据文件存在但内容不完整时，部分业务仍可能继续使用内置默认样例，与 `--allow-fallback-sample` 的严格失败设计不完全一致。

## 4. 阻塞验收问题

### P1-1：CLI 覆盖 Profile 的语义不可靠，`--batch-size` 不限制执行批量
...
### P1-2：显式 Control 文件读取或下发失败仍被当作成功
...
### P1-3：Business 与 Pipeline 的预检使用子串匹配，存在误放行
...
RFC-0005 已实现主要结构性目标，代码解耦和可扩展性相较原单文件、数字枚举式 Demo 有明显改善，且未破坏四层架构与公开 ABI。当前剩余问题集中在通用 Runner 的配置优先级、错误传播和业务配置校验上，属于可在后续小范围修复中关闭的问题。

在 3 项 P1 关闭前，建议维持“已合入、待验收修复”的状态，不建议对外宣称 RFC-0005 已完整验收通过。
