...
6. Record any non-default sanitizer, real-model, hardware, performance, or compatibility evidence required by the task or current contracts. Do not substitute it for the canonical gate.
7. Update durable documentation according to the
   [design and current-contract policy](../../../../CONTRIBUTING.md#3-design-and-current-contracts).
   Verify against current guides and affected code/tests. Remote delivery remains out of scope
   unless explicitly requested; if requested, use `github-branch-merge`.
...
when business effects or target-environment acceptance is in scope. Follow the root
[repository guardrails](../../../../AGENTS.md#repository-guardrails) for internal-SDK restrictions
and migration boundaries; ordinary development does not require all production acceptance work.
