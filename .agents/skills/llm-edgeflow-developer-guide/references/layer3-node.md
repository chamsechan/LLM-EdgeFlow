...
3. Inherit `INode`; keep per-request state exclusively in `AlgContext`. Members may hold immutable configuration or safe shared handles, and the Definition must truthfully declare parallel safety.
4. Declare inputs/outputs with the same `BlackboardKey<T>` objects used by `Process`. Never guess or duplicate key strings with inconsistent types.
