...
- Preserve legacy sequential parsing. Convert to explicit `id + depends_on` only through `PipelineValidator::NormalizeExplicitDag` or `alg_pipeline_tool normalize`.
...
- Avoid request-data copies and upward dependencies from Core into business nodes or concrete engines.
