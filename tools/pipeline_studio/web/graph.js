...
export function layeredPositions(nodes) {
  ...
  for (const node of nodes) {
    ...
    positions[node.id] = { x: 65 + depth * 400, y: 60 + row * 310 };
...
export class GraphView {
  ...
  bindPanZoom() {
    ...
    window.addEventListener("pointermove", event => {
      ...
      if (this.connecting) {
        ...
        const source = this.positions[this.connecting.nodeId];
    ...
    window.addEventListener("pointerup", event => {
      ...
      if (this.connecting) {
        ...
        if (inputPort && inputPort.dataset?.nodeId && inputPort.dataset.nodeId !== this.connecting.nodeId) {
          this.callbacks.connect(this.connecting.nodeId, this.connecting.port, inputPort.dataset.nodeId, inputPort.dataset.port);
        }
  ...
  layout(nodes, force = false) {
    ...
    if (positionsChanged) {
      ...
      if (this.root && nodes.length) this.fit();
    }
  }

  fit() {
    const positions = Object.values(this.positions);
    const width = Math.max(...positions.map(point => point.x + WIDTH)) + 50;
    const height = Math.max(...positions.map(point => point.y + 280)) + 40;
    this.scale = Math.max(.35, Math.min(1, this.root.clientWidth / width, this.root.clientHeight / height));
    this.offset = { x: 0, y: 0 }; this.transform();
  }
  ...
  render(nodes, selectedId, errorIds = new Set(), modelIds = new Set(), definitions = {}, edges = []) {
  ...
  renderEdges(nodes) {
    ...
    for (const binding of this.edges || []) {
      const from = this.positions[binding.source], to = this.positions[binding.target];
      if (!from || !to) continue;
      const y1 = binding.dependency ? 38 : this.portY(binding.source, "outputs", binding.sourcePort);
      const y2 = binding.dependency ? 38 : this.portY(binding.target, "inputs", binding.targetPort);
      const edge = svg("path", { d: this.curve(from.x + WIDTH, from.y + y1, to.x, to.y + y2) });
      if (binding.dependency) edge.setAttribute("stroke-dasharray", "5 5");
      const title = svg("title"); title.textContent = binding.dependency ? "执行依赖" : `${binding.sourcePort} → ${binding.targetPort}`;
      edge.append(title);
      edge.addEventListener("click", () => this.callbacks.deleteEdge(binding));
      this.edgeLayer.append(edge);
    }
  }

  portY(id, direction, name) {
    return 76 + Math.max(0, (this.definitions[id]?.[direction] || []).findIndex(port => port.key === name)) * 26;
  }
  ...
  nodeElement(node, selected, hasError = false, hasModel = false) {
    ...
    const definition = this.definitions[node.id] || {};
    const height = Math.max(HEIGHT, 90 + Math.max(definition.inputs?.length || 0, definition.outputs?.length || 0) * 26);
    group.append(svg("rect", { class: "body", width: WIDTH, height }));
    ...
    for (const [direction, ports] of [["input", definition.inputs || []], ["output", definition.outputs || []]]) {
      ports.forEach((port, index) => {
        const y = 76 + index * 26;
        const circle = svg("circle", { class: `port ${direction}`, cx: direction === "input" ? 0 : WIDTH, cy: y, r: 7 });
        circle.dataset.nodeId = node.id; circle.dataset.port = port.key;
        const tooltip = svg("title"); tooltip.textContent = `${port.key}: ${port.type_id} · ${port.cardinality || ""}`; circle.append(tooltip);
        const label = svg("text", { class: "port-label", x: direction === "input" ? 13 : WIDTH - 13, y: y + 4, "text-anchor": direction === "input" ? "start" : "end" });
        label.textContent = `${port.key}${port.required && direction === "input" ? " *" : ""}`;
        if (direction === "input") circle.addEventListener("pointerup", event => {
          event.stopPropagation();
          if (this.connecting) this.callbacks.connect(this.connecting.nodeId, this.connecting.port, node.id, port.key);
          this.cancelConnection();
        });
        else circle.addEventListener("pointerdown", event => {
          event.stopPropagation(); this.connecting = { nodeId: node.id, port: port.key, y };
        });
