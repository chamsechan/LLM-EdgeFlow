...
1. 选择业务契约 `keyword_match_v1`，从 `keyword_match_mock` Profile 克隆并点击“新建”。
2. 在画布检查“业务输入”的 `input_sentences` → 节点 `text`，以及节点 `matches` →
   “业务输出”的 `rule_matches`。可点击一条连线删除，再从输出端口拖到输入端口重新连接。
3. 选择规则节点，在属性中将 `categories` 改为 `{"FIRST_RUN":["VIP"]}` 并点击“应用”。
   在“原始 JSON”确认端口映射；需要模型的方案则先在“模型”页应用实例，再在节点属性绑定它。
